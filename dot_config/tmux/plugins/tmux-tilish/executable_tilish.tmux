...
# Open a file manager with Alt + Enter.
# if [ -z "$legacy" ]
# then
# 	tmux $bind "${mod}e" \
#     run-shell "~/bin/ranger/ranger.sh"
#    run-shell 'pid="`tmux display -p "#{pane_pid}"`";cwd="`ls -l /proc/$pid/cwd | grep -oP "\-\> \K.*"`"; tmux select-pane -t "bottom-right"; tmux split-pane "echo $cwd >> /home/daniel/out.txt ;export EDITOR=nvim VISUAL=nvim; source ranger ranger; zsh"'
# else
# 	tmux $bind "${mod}e" \
#     select-pane -t 'bottom-right' \\\;\
#     split-window '. ranger;bash' \\\;\
#     run-shell 'tmux selection-layout' \\\;\
#     send escape
# fi
