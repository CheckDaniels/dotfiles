...
class tmux_split_into_ranger(Command):
    """
    :tmux_split_into_ranger
    Split the tmux window and change the directory to the current path of the ranger session (starting new ranger session).
    """
    def execute(self):
        current_dir = self.fm.thisdir.path
        tmux_command = f"tmux split-window -t tmux:0 -c {current_dir} 'export EDITOR=nvim VISUAL=nvim;. ranger;zsh'"
        os.system(tmux_command)

class tmux_split_into_zsh(Command):
    """
    :tmux_split_into_zsh
    Split the tmux window and change the directory to the current path of the ranger session (staring new zshell session).
    """
    def execute(self):
        current_dir = self.fm.thisdir.path
        tmux_command = f"tmux split-window -t tmux:0 -c {current_dir}"
        os.system(tmux_command)
